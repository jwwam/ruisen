<template>
	<el-drawer v-model="visible" :title="$t('personal.name')" size="40%">
		<el-tabs class="personal-tabs">
			<el-tab-pane label="基本信息" v-loading="loading">
				<el-form :model="formData" :rules="ruleForm" label-width="100px" class="mt30" ref="formdataRef" style="padding: 20px">
					<el-row :gutter="20">
						<el-col :span="24" class="mb20">
							<el-form-item prop="avatar">
								<ImageUpload v-model:imageUrl="formData.avatar" borderRadius="50%" style="width: 120px; height: 120px">
									<template #empty>
										<el-icon>
											<Avatar />
										</el-icon>
...
const initUserInfo = (userId: any) => {
	...
	getObj(userId)
		...
		.catch((err) => {
			useMessage().error(err.msg || '获取用户信息失败');
		})
		.finally(() => {
			loading.value = false;
		});
