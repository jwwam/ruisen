...
package com.ruisen.rsmanage.common.xss.core;

import cn.hutool.core.util.CharsetUtil;
import com.ruisen.rsmanage.common.xss.config.RsmanageXssProperties;
import com.ruisen.rsmanage.common.xss.utils.XssUtil;
...
public class DefaultXssCleaner implements XssCleaner {
	...
	private final RsmanageXssProperties properties;

	public DefaultXssCleaner(RsmanageXssProperties properties) {
		this.properties = properties;
	}

	private static Document.OutputSettings getOutputSettings(RsmanageXssProperties properties) {
	...
	public String clean(String bodyHtml, XssType type) {
		...
		RsmanageXssProperties.Mode mode = properties.getMode();
		if (RsmanageXssProperties.Mode.escape == mode) {
			// html 转义
			return HtmlUtils.htmlEscape(bodyHtml, CharsetUtil.UTF_8);
		}
		else if (RsmanageXssProperties.Mode.validate == mode) {
			// 校验
